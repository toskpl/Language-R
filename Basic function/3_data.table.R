...
iris_data_dt[species == 'virginica', mean(petal_length)]
###################################################
###################################################
dt = data.table(
  id = 1:1000,
...
### Oblicz średnią pensję na danym stanowisku, dzięląc jednocześnie pracowaników wg. liczby pełnych 10-tek liczby ich lat stażu.
### Wynik posortuj wg. nazwy stanowiska i liczby pełnych dekad stażu.

dt[, dekada := ceiling(staz/10)]
dt[, .(mean = mean(pensja),.N), keyby = .(stanowisko, dekada)]


# ale jeżeli byśmy chcieli aby stanowiska były sortowane wg. ich "ważności":
dt[, stanowisko := factor(stanowisko, levels = c("prezes", "dyrektor", "menedzer", "specjalista"), ordered = T)]
